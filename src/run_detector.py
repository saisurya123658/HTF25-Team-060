...
# lane_lines = [200, 400]      # example lane positions for wrong-lane detection
# speed_limit = 30             # pixels/frame for overspeeding
...
# clip_frames = {}  # store frames for short clips
# frame_count = 0
# ocr_skip = 5       # Run OCR every 5 frames
# helmet_skip = 2    # Run helmet detection every 2 frames

# # --- Function to save clip asynchronously ---
...
#         label = int(cls)
#         if label in [0,1,2,3,5,7]:  # person, bicycle, car, motorbike, bus, truck
...
#         # --- Read license plate (every ocr_skip frames) ---
...
#         # 4. Triple riding and helmet
#         if label == 3: # motorbike
#             results_person = [int(cls) for cls in results.boxes.cls if int(cls)==0]
#             if len(results_person) > 1:
#                 violation_type = "triple_riding"

#             # --- Helmet detection every helmet_skip frames ---
...
#             # Save short clip asynchronously
...
#     cv2.imshow("AI Traffic Detector", frame)
...
# print(f"[INFO] Detection finished. Total violations saved in {json_file}")
